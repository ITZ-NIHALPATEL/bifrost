...
// Resolve a datasheet entry for a logged (model, provider). Datasheet keys are
// model ids — bare ("gpt-4o", "claude-haiku-4-5") or prefixed
// ("anthropic.claude-...", "azure/gpt-..."). Try a few normalizations; return
// null when there's no confident match so the caller skips the accuracy half.
...
// AI-Studio variant "gemini/<model>" with provider "gemini"), so a Bifrost "vertex"
// row would otherwise fail the provider guard below.
...
function resolvePricingEntry(sheet, model, provider) {
  ...
  const candidates = [m, bare,
    provider ? `${provider}/${bare}` : null,
    np && np !== String(provider).toLowerCase() ? `${np}/${bare}` : null,
  ].filter(Boolean);
  ...
  for (let i = 0; i < delays.length; i++) {
    ...
    try {
      ...
      if (rows && rows.length > 0) {
        row = rows[0];
        if (row.cost != null && Number(row.cost) > 0) break; // cost landed; stop early
      }
    } catch (e) {
  ...
  newman.on('request', function (err, args) {
    ...
    if (expectCost && /^(1|true|yes)$/i.test(String(expectCost))) {
      ...
      if (!logsDbReady || !logsDb) {
        results.push({ name, result: 'SKIP', detail: 'Logs DB not connected (costing check)' });
        return;
      }
